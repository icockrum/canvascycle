(function(global){
  ...
  function normalizeData(input) {
    return {
      ...
      colors: (input.colors || []).map(function(c){ return [c[0], c[1], c[2]]; }),
  ...
  function getCycledColors(baseColors, cycles, timeNow) {
    var colors = baseColors.map(function(c){ return [c[0], c[1], c[2]]; });
  ...
  function CanvasCyclePlayer(canvas) {
    this.canvas = canvas;
  ...
  CanvasCyclePlayer.prototype.loadFromData = function(jsonData) {
  ...
  CanvasCyclePlayer.prototype.loadFromUrl = async function(url) {
  ...
  CanvasCyclePlayer.prototype.pause = function() { this.paused = true; };
  CanvasCyclePlayer.prototype.resume = function() { this.paused = false; };
  CanvasCyclePlayer.prototype.stop = function() {
    this.paused = true;
  ...
  CanvasCyclePlayer.prototype.loop = function(now) {
  ...
  CanvasCyclePlayer.prototype.render = function(now) {
    ...
    for (var i = 0; i < src.length; i++) {
      ...
      var di = i * 4;
      dst[di] = c[0]; dst[di + 1] = c[1]; dst[di + 2] = c[2]; dst[di + 3] = 255;
    }
    this.ctx.putImageData(this.imageData, 0, 0);
  };

  global.CanvasCyclePlayer = CanvasCyclePlayer;
})(window);
