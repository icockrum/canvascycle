...
var CanvasCycle = {
	...
	openPaletteColorPicker: function (idx, anchorEl) {
		...
		if (anchorEl && anchorEl.getBoundingClientRect) {
			...
			this.paletteColorInputEl.style.left =
				Math.max(0, Math.round(r.left)) + "px";
			this.paletteColorInputEl.style.top =
	...
	bindMenus: function () {
		...
		appTrigger.addEventListener("click", function (e) {
			e.stopPropagation();
			CanvasCycle.toggleMenu(
				appMenu,
				appTrigger,
				[fileMenu, editMenu, helpMenu],
				[fileTrigger, editTrigger, helpTrigger],
			);
		});
		...
		fileTrigger.addEventListener("click", function (e) {
			...
			CanvasCycle.toggleMenu(
				fileMenu,
				fileTrigger,
				[appMenu, editMenu, helpMenu],
				[appTrigger, editTrigger, helpTrigger],
			);
		});
		...
		editTrigger.addEventListener("click", function (e) {
			...
			CanvasCycle.toggleMenu(
				editMenu,
				editTrigger,
				[appMenu, fileMenu, helpMenu],
				[appTrigger, fileTrigger, helpTrigger],
			);
		});
		...
		helpTrigger.addEventListener("click", function (e) {
			...
			CanvasCycle.toggleMenu(
				helpMenu,
				helpTrigger,
				[appMenu, fileMenu, editMenu],
				[appTrigger, fileTrigger, editTrigger],
			);
		});
	...
	updateZoomDrag: function (e) {
		...
		nextZoom = Math.max(
			this.view.minZoom,
			Math.min(this.view.maxZoom, nextZoom),
		);
